 ...
 */
const sendfile = (res: ServerResponse, file: send.SendStream, options: Object, callback: Function) => {
  let done = false
  ...
  if ((options as any).headers) {
    ...
    file.on('headers', function(res: ServerResponse) {
...
const acceptParams = (str: string, index?: number) => {
  let parts = str.split(/ *; */)
  let ret = { value: parts[0], quality: 1, params: <OBJ>{}, originalIndex: index }
...
res.send = function(body) {
  ...
  switch (typeof chunk) {
    ...
    case 'object':
      ...
      else return this.json(<Object>chunk)
  }
  ...
  if (chunk !== undefined) {
    if (Buffer.isBuffer(chunk)) {
      ...
      len = chunk.length
    } else if ((chunk as string).length < 1000) {
...
res.sendStatus = function(statusCode) {
  let body = STATUS_CODES[statusCode] || String(statusCode)
